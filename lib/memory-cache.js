...
MemoryCache.prototype.add = function(entries, callback) {
  _.each(entries, function(addElement) {
    var alreadyAdded = _.some(this._entries, function(element) {
      return _.isEqual(element, addElement);
    });

    // Skip any entries that are duplicates of the existing
    // cache elements.
    if (alreadyAdded) {
      return;
    }
